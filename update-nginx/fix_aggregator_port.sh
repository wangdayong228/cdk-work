...
resolve_container_by_enclave() {
  ...
  endpoint=$(kurtosis port print "$ENCLAVE_NAME" "$SERVICE_NAME" aggregator 2>/dev/null || true)
  if [ -z "$endpoint" ]; then
    return 1
  fi
...
if [ -z "${CONTAINER_NAME}" ]; then
  if [ -n "$ENCLAVE_NAME" ]; then
    echo "WARN: 未找到匹配容器（enclave=${ENCLAVE_NAME}, pattern=${CONTAINER_PATTERN}），跳过 aggregator DNAT 配置。"
  else
    echo "WARN: 未找到匹配容器（pattern=${CONTAINER_PATTERN}），跳过 aggregator DNAT 配置。"
  fi
  exit 0
fi
...
if [ -z "${CONTAINER_IP}" ]; then
  echo "ERROR: 无法获取容器 IP: ${CONTAINER_NAME}"
  exit 1
fi

echo "Aggregator 容器: ${CONTAINER_NAME} (${CONTAINER_IP}:${CONTAINER_PORT})"
...
echo "Fixed aggregator port ${HOST_PORT} -> ${CONTAINER_IP}:${CONTAINER_PORT}"
